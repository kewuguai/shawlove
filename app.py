...
# **🔥 逐字动画新逻辑，使用 `st.empty()` 逐步更新**
def type_text(placeholder, text, speed=0.3):
    output = ""
    for char in text:
        output += char
        placeholder.markdown(f"<p class='question'>{output}</p>", unsafe_allow_html=True)
        time.sleep(speed)
...
def show_final_result():
    ...
    type_text(answer_placeholder, "王喆", 0.6)  # **🔥 答案逐字显示更慢**

# **🔥 确保 `show_intro()` 只有在 `if __name__ == "__main__"` 时执行**
